...
while (t < (Options.T-0.001))
    ...
    if(size(u,3)<4) % Check if 2D or 3D
        % Do a diffusion step
        if(strcmpi(Options.Scheme,'R')&&(Options.eigenmode==9)&&(exist('CoherenceFilterStep2D')==3))
...
% ----------------
% Experimental Test Section with Different eigenvalues

% [uxx, uxy, uyy] = derivative7(usigma,'xx','xy','yy');
% [L1,L2,lv1x,lv1y,lv2x,lv2y]=EigenVectors2DLap(uxx,uxy,uyy);
% [Dxx,Dxy,Dyy]=ConstructDiffusionTensor2D(L1,L2,v1x,v1y,v2x,v2y,gradA,Options);
% ----------------
